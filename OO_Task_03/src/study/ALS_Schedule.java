            ...
            for (int i = 1; i < follow.getRindex(); i++) {  //start from the first carry req
                ...
                if (eva1.getE_floor() == carryBuffer.getDfloor()) {
                    ...
                    for (int j = 1; j < follow.getRindex(); j++) //delete the request in the same floor
                        if (e_floor == follow.getRequest(j).getDfloor()) follow.setDone(j);
                    portering = 2;
                }
            }

            //finish primary
            if (primary.getDfloor() > e_floor) eva1.move(Moveable.State.UP);
            else if (primary.getDfloor() < e_floor) eva1.move(Moveable.State.DOWN);
