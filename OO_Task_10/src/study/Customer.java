...
import static study.Main.taxiNum;
...
public class Customer extends Thread {
	...
	public static void block(int x, int y, Direction dir) {
	...
	/**
	 * 用来获取编号为a的超级出租车的第b个请求的Iterator的方法
	 *
	...
	//Require : a,b > 0
	//Modified : null
	//Effect : 用来获取用来获取编号为a的超级出租车的第b个请求的Iterator
	//          当a小于普通出租车数目taxiNum会输出报错信息,并终止程序
	//          b大于a车完成的请求数的时候回报错，并终止程序
	public ListIterator getIterator(int a, int b) {
		if (a < taxiNum) {
			System.out.println("the ID you entered is not a supertaxi ID! Please try again!");
			System.exit(0);
			return null;
		}
		else {
			SuperTaxi st = (SuperTaxi) taxis[a];
			return st.getIt(b);
		}
	}

	/**
	 * 用来打印编号为a的超级出租车的第b个请求运行路径信息的方法
	 *
	 * @param a 所要查询的超级出租车的编号
	 * @param b 所要查找的请求编号
	 */
	//Require : a,b > 0
	//Modified : null
	//Effect : 用来打印编号为a的超级出租车的第b个请求运行路径信息的方法,
	//          当a小于普通出租车数目taxiNum会输出报错信息
	//          b大于a车完成的请求数的时候回报错，并终止程序
	public void showServInfo(int a, int b) {
		if (a < taxiNum) {
			...
			ListIterator<Node> it = getIterator(a, b);
			while (it.hasNext()) {
				System.out.println("Super-Taxi " + a + ": " + it.next().toString());
			}
		}
	}
