...
public class Map {
	...
	private static Node[][] orimap = new Node[MAXCOUNT][MAXCOUNT];
	 ...
	 * 获得原始地图(未经过任何修改)的方法
	 *
	 * @return 返回Node[][]形式的原始地图
	 */
	//Require : Map is initialized after buildPointSet(),buildMatrix,buildVertical
	//Modified : null
	//Effect : 返回Node[][]形式的原始地图,主要供SuperTaxi使用
	public static Node[][] getOrimap() {
		return orimap;
	}
				...
				for (int j = 0; j < MAXCOUNT; j++) {
					switch (pointset[i][j]) {
						...
						case '1':
							...
							orimap[j][i].linkright();
							matrix[j + 1][i].linkleft();
							orimap[j + 1][i].linkleft();
							break;
						case '2':
							matrix[j][i].linkdown();
							orimap[j][i].linkdown();
	...
	public static synchronized ArrayDeque<Node> findSSp(Node start, Node end) {
...
//		map.blocktheWay(3, 5, Direction.RIGHT);
		map.blocktheWay(3, 4, Direction.DOWN);
